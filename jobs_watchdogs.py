def create_jobs_watchdogs(deps: dict):
    ...
    PRIME_TIME_LINES = [
        ...
        "Zufall trifft voll: {user} räumt den Prime-Time Pot mit +{coins} Coins ab.",
            ...
            for user_id, username, started_ts, expires_ts, remind_stage in rows:
                ...
                if expires_ts <= now:
                    ...
                    msg = (
                        ...
                        "Groß labern, nichts liefern. Der ganze Chat sieht jetzt schwarz auf weiß, dass außer heißer Luft, Rückgrat aus Pappe und erbarmungsloser Fremdscham wirklich gar nichts da ist."
                    )
                    try:
                        await context.bot.send_message(chat_id=chat_id, text=msg, parse_mode=ParseMode.HTML)
                    except Exception:
                ...
                if remind_stage == 0 and now >= started_ts + LOVE_REMIND_2_S:
                    ...
                    msg = (
                        ...
                        "Wenn du jetzt wieder nichts lieferst, reißt es dir gleich brutal die Hälfte deiner Coins weg und der ganze Chat sieht, was für ein Totalausfall du bist."
                    )
                    try:
                        await context.bot.send_message(chat_id=chat_id, text=msg, parse_mode=ParseMode.HTML)
                    except Exception:
                ...
                if remind_stage == 0 and now >= started_ts + LOVE_REMIND_1_S:
                    ...
                    msg = (
                        ...
                        "Bis jetzt kommt von dir nur große Fresse und null Leistung. Wenn das so bleibt, wird dir die Challenge die Coins aus der Tasche treten."
                    )
                    try:
                        await context.bot.send_message(chat_id=chat_id, text=msg, parse_mode=ParseMode.HTML)
                    except Exception:
    ...
    async def runaway_watchdog_job(context):
        ...
        async with aiosqlite.connect(deps["DB"]) as db:
            ...
            for pet_id, owner_id, acquired_ts, pet_username, owner_username in rows:
                ...
                care_window = await _care_count_last_24h(db, chat_id, pet_id_i, owner_id_i, now)
